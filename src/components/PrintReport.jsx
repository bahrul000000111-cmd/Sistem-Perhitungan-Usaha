/**
 * PrintReport.jsx
 * Hidden print-only report layout. Rendered off-screen and shown via window.print().
 * Contains a full summary of all records with calculations.
 */
...
export default function PrintReport({ records }) {
  ...
  return (
    <div className="print-only" style={{ display: 'none' }} id="print-report">
      ...
      {/* This component is only visible when window.print() is called via CSS */}
      <div className="p-8 max-w-4xl mx-auto font-sans text-black">
        ...
        <div className="text-center border-b-2 border-blue-800 pb-6 mb-6">
          ...
          <p className="text-sm text-gray-600 mt-1">Usaha Mikro Kecil — Berdasarkan Koefisien Normatif Pemerintah</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="border border-gray-200 rounded p-4">
            <p className="text-xs text-gray-500 uppercase">Total Usaha Terkelola</p>
            <p className="text-2xl font-bold text-blue-800">{stats.totalUMK} Usaha</p>
          </div>
          <div className="border border-gray-200 rounded p-4">
            <p className="text-xs text-gray-500 uppercase">Total Hasil Usaha Bersih (Tahunan)</p>
            <p className="text-2xl font-bold text-green-700">{formatRupiah(stats.totalNetAnnual)}</p>
          </div>
          <div className="border border-gray-200 rounded p-4">
            <p className="text-xs text-gray-500 uppercase">Total Pendapatan Kotor (Tahunan)</p>
            <p className="text-xl font-bold text-indigo-700">{formatRupiah(stats.totalRevenue)}</p>
          </div>
          <div className="border border-gray-200 rounded p-4">
            <p className="text-xs text-gray-500 uppercase">Rata-rata Pendapatan Per Bulan/Usaha</p>
        ...
        {records.map((record, idx) => {
          ...
          return (
            <div key={record.id} className="mb-6 border border-gray-200 rounded p-4" style={{ pageBreakInside: 'avoid' }}>
              ...
              <table className="w-full text-[12px] border-collapse">
                <thead>
                  <tr className="bg-gray-100">
                    <th className="border border-gray-200 px-3 py-1.5 text-left font-semibold">Komponen</th>
              ...
              {/* Formula note */}
              <p className="text-[10px] text-gray-400 mt-2">{category?.note}</p>
            </div>
          );
        })}
